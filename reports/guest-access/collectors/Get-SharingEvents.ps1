...
<#
    ...
    .DESCRIPTION
        ...
        window gets its own session. A window that reaches the cap is logged as possibly
        truncated; re-run it with a smaller -WindowHours.
...
foreach ($window in Split-DateRange -Start $start -End $end -WindowMinutes ($WindowHours * 60)) {
    ...
    while ($collected -lt $sessionCap) {
        try {
            $page = @(Search-UnifiedAuditLog -StartDate $window.Start -EndDate $window.End `
    ...
    if ($collected -ge $sessionCap) {
        Write-CollectorLog -OutputPath $OutputPath -Level Warning -Source $source -Message (
            'The window {0} to {1} reached the 50,000-record session cap and may be truncated. Re-run it with a smaller -WindowHours.' -f
