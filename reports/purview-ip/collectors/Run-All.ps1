...
<#
    .SYNOPSIS
        Runs the Entra user collector and all four Purview information
        protection collectors into one output folder.

    .DESCRIPTION
        Each collector signs in for itself, so a collector whose service is
        unavailable in this cloud or unlicensed in this tenant leaves a
    ...
    .PARAMETER Organization
        ...
        Exchange Online, which Get-CopilotAccessedResources.ps1 uses.
...
Write-CollectorLog -OutputPath $OutputPath -Source 'run-all' -Message (
    'Starting the Purview information protection collectors against the {0} cloud.' -f $Environment)

$steps = @(
    @{ Name = 'users'; Script = $null }
    @{ Name = 'policies'; Script = 'Get-Policies.ps1' }
    @{ Name = 'activity-explorer-events'; Script = 'Get-ActivityExplorerEvents.ps1' }
    @{ Name = 'content-explorer-snapshot'; Script = 'Get-ContentExplorerSnapshot.ps1' }
    @{ Name = 'copilot-accessed-resources'; Script = 'Get-CopilotAccessedResources.ps1' }
)
...
foreach ($step in $steps) {
    try {
        if ($null -eq $step.Script) {
            ...
            & (Join-Path $PSScriptRoot $step.Script) -OutputPath $OutputPath @auth
        }
    }
    catch {
